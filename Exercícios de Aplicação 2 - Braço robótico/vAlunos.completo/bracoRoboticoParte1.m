...
while err > precisao && i <= iMax
    ...
    % REMOVER
    f = L1*cos(alpha) + L2*cos(beta) + L3*cos(gama) - xAlvo;
    % REMOVER
    g = L1*sin(alpha) + L2*sin(beta) + L3*sin(gama) + Altura - yAlvo;
    % REMOVER
    h = gama - gamaAlvo;
...
% REMOVER
teta1Graus = alphaGraus; 
teta2Graus = betaGraus - alphaGraus;
teta3Graus = gamaGraus - betaGraus;

% REMOVER
if (teta1Graus >= -135 && teta1Graus <= 135 &&...
    teta2Graus >= -135 && teta2Graus <= 135 &&...
    teta3Graus >= -135 && teta3Graus <= 135)
...
% REMOVER
braco3BaseX = braco2PontaX;
braco3BaseY = braco2PontaY;

% REMOVER
braco3PontaX = braco3BaseX + L3 * cos(gama);
braco3PontaY = braco3BaseY + L3 * sin(gama);
